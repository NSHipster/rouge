  ...
  module Lexers
    class Swift < RegexLexer
      ...
      state :root do
        ...
        rule %r{[()\[\]{}:;,?\\]}, Punctuation
        rule %r([-/=+*%<>!&|^.~]+), Operator
        rule %r/#"/, Str, :raw_string_literal
        rule %r/"""/, Str, :multiline_string_literal
        rule %r/@?"/, Str, :string_literal
        rule %r/'(\\.|.)'/, Str::Char
        ...
        rule %r/(?!\b(private|fileprivate|internal|public|unowned)\b)\b#{id}(?=(\?|!)?\s*[(])/ do |m|
          if m[0] =~ /^[[:upper:]]/
            token Keyword::Type
          else
            token Name::Function
